...
void OnTimer()
{
   ...
   // Step 8: Manage open positions (TP1 check, partial close, BE shift)
   ManageOpenPositions();

   // Step 9: Update dashboard/comment
   UpdateDashboard();
...
/**
 * CalculateFibTP - Calculate take-profit level based on Fibonacci extension
 * Based on specification Chapter 2.6 - Take-Profits
 *
 * Long Setup:
 ...
 *
 * @param symbolIndex - Index in symbolStates array
 * @param extension - Fibonacci extension multiplier (1.000, 1.272, 1.618)
...
double CalculateFibTP(int symbolIndex, double extension)
{
   ...
   if (swingHigh <= 0 || swingLow <= 0) return 0;
...
bool OpenTrade(int symbolIndex)
{
   ...
   if (success) {
      ...
      Print("[OpenTrade] SUCCESS | ", symbol, " | Type: ", (isLong ? "LONG" : "SHORT"),
            " | Lot: ", lotSize, " | Entry: ", entry, " | SL: ", sl, " | TP1: ", tp1);

      // Log trade to file
      LogTradeToFile(symbol, isLong, entry, sl, tp1, lotSize);
...
/**
 * ManageOpenPositions - Manage all open positions
 * Based on specification Chapter 6.1 - Partial Close at TP1
 *
 * Process:
 * 1. Iterate through all open positions
 * 2. Check if TP1 is reached → Close 50% + Move SL to BE + Set TP to TP2
 * 3. Check if TP2 is reached → Update TP to TP3
 * 4. TP3 remains as final target
 * 5. Flag positions so each TP is only processed once
 */
void ManageOpenPositions()
{
   for (int i = PositionsTotal() - 1; i >= 0; i--) {
      ...
      // Check TP1
      string tp1FlagName = "EA_TP1_" + IntegerToString(ticket);
      if (!GlobalVariableCheck(tp1FlagName)) {
         ...
         if (tp1Reached) {
            Print("[ManageOpenPositions] TP1 reached for ticket ", ticket, " at price ", currentPrice);

            // Close 50% of position
            ClosePartial(ticket, PartialClosePercent / 100.0);

            // Move remaining SL to Break-Even and set TP to TP2
            MoveToBreakEven(ticket);

            // Set flag so TP1 is not processed again
            GlobalVariableSet(tp1FlagName, 1);
         }
      }

      // Check TP2 (only if TP1 was already hit)
      string tp2FlagName = "EA_TP2_" + IntegerToString(ticket);
      if (GlobalVariableCheck(tp1FlagName) && !GlobalVariableCheck(tp2FlagName)) {
         ...
         if (tp2Reached) {
            Print("[ManageOpenPositions] TP2 reached for ticket ", ticket, " at price ", currentPrice);

            // Update TP to TP3
            int symbolIndex = GetSymbolIndexFromTicket(ticket);
            if (symbolIndex >= 0) {
               double tp3 = CalculateFibTP(symbolIndex, 1.618);
               if (tp3 > 0) {
                  double currentSL = PositionGetDouble(POSITION_SL);
                  CTrade trade;
                  trade.PositionModify(ticket, currentSL, tp3);

                  if (trade.ResultRetcode() == TRADE_RETCODE_DONE) {
                     Print("[ManageOpenPositions] TP updated to TP3: ", tp3, " for ticket ", ticket);
                  } else {
                     Print("[ManageOpenPositions] Failed to update TP to TP3 for ticket ", ticket);
                  }
               }
            }

            // Set flag so TP2 is not processed again
            GlobalVariableSet(tp2FlagName, 1);
         }
      }
   }
}
...
/**
 * ClosePartial - Close a portion of an open position
 * Based on specification Chapter 6.1
 *
 * @param ticket - Position ticket
 * @param percent - Percentage to close (0.5 for 50%)
 */
void ClosePartial(ulong ticket, double percent)
{
   ...
   if (trade.ResultRetcode() == TRADE_RETCODE_DONE) {
      Print("[ClosePartial] Success | Ticket: ", ticket, " | Volume: ", closeVolume);

      // Calculate profit percentage
      double profitPercent = (profit > 0 && openPrice > 0) ? ((currentPrice - openPrice) / openPrice * 100) : 0;

      // Log closed trade (TP1 status)
      LogClosedTrade(symbol, isLong, currentPrice, profit, profitPercent, 0, "TP1");
   } else {
      Print("[ClosePartial] Failed | Ticket: ", ticket, " | Error: ", trade.ResultRetcodeDescription());
   }
}

/**
 * GetSymbolIndexFromTicket - Find symbol index for a position ticket
 *
 * @param ticket - Position ticket
 * @return Symbol index or -1 if not found
 */
int GetSymbolIndexFromTicket(ulong ticket)
{
   ...
   for (int i = 0; i < ArraySize(symbolStates); i++) {
      if (symbolStates[i].symbol == posSymbol) {
         return i;
      }
   }
...
/**
 * MoveToBreakEven - Move stop-loss to break-even and update TP to TP2
 * Based on specification Chapter 6.2 - Break-Even Shift
 *
 * Process:
 * 1. Get open price (break-even level)
 * 2. Check if SL is not already at break-even
 * 3. Calculate TP2 level
 * 4. Modify position with new SL = open price and TP = TP2
 * 5. Only executed after TP1 hit (via ManageOpenPositions)
 *
 * @param ticket - Position ticket
 */
void MoveToBreakEven(ulong ticket)
{
   ...
   // Get symbol index to calculate TP2
   int symbolIndex = GetSymbolIndexFromTicket(ticket);

   // Calculate TP2 (1.272 extension)
   double tp2 = (symbolIndex >= 0) ? CalculateFibTP(symbolIndex, 1.272) : 0;

   // If we can't calculate TP2, keep current TP
   if (tp2 <= 0) {
      tp2 = currentTP;
      Print("[MoveToBreakEven] Warning: Could not calculate TP2, keeping current TP for ticket ", ticket);
   }
   ...
   if (shouldMove) {
      CTrade trade;
      trade.PositionModify(ticket, openPrice, tp2);

      if (trade.ResultRetcode() == TRADE_RETCODE_DONE) {
         Print("[MoveToBreakEven] Success | Ticket: ", ticket, " | New SL: ", openPrice, " | New TP: ", tp2, " (TP2)");
      } else {
         Print("[MoveToBreakEven] Failed | Ticket: ", ticket, " | Error: ", trade.ResultRetcodeDescription());
      }
   } else {
      Print("[MoveToBreakEven] SL already at or above break-even for ticket ", ticket);
   }
}
 ...
 *
 * CSV Format:
 * Timestamp,Symbol,Type,Entry,SL,TP1,Lot,RiskPercent,Equity,DD
 *
 ...
 */
bool LogTradeToFile(string symbol, bool isLong, double entry, double sl, double tp1, double lotSize)
{
   ...
   if (isNewFile) {
      FileWrite(handle, "Timestamp,Symbol,Type,Entry,SL,TP1,Lot,RiskPercent,Equity,DD");
   }
   ...
   FileWrite(handle, timestamp, symbol, tradeType,
             DoubleToString(entry, 5), DoubleToString(sl, 5), DoubleToString(tp1, 5),
 ...
 * @param status - "TP1", "TP2", "TP3", "SL", "MANUAL"
 * @return true if logging successful
 */
